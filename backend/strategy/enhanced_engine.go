...
// EnhancedStrategyEngine 增强版策略引擎
type EnhancedStrategyEngine struct {
...
// NewEnhancedStrategyEngine 创建增强版策略引擎
func NewEnhancedStrategyEngine() *EnhancedStrategyEngine {
...
func (e *EnhancedStrategyEngine) ExecuteStrategy(strategyID string, tradeDate time.Time) error {
	// 获取策略配置
	...
	// 流动性筛选（日均成交额≥2亿）
	liquidStocks := e.filterByLiquidity(stocks, tradeDate, 200000000)
	log.Printf("流动性筛选后剩余 %d 只股票", len(liquidStocks))
	...
	case "short_term":
		results, err = e.executeShortTermStrategy(strategy, liquidStocks, tradeDate)
	case "medium_term":
		results, err = e.executeMediumTermStrategy(strategy, liquidStocks, tradeDate)
	case "long_term":
		results, err = e.executeLongTermStrategy(strategy, liquidStocks, tradeDate)
	default:
...
func (e *EnhancedStrategyEngine) executeShortTermStrategy(strategy *model.Strategy, stocks []model.Stock, tradeDate time.Time) ([]model.StrategyResult, error) {
	var results []model.StrategyResult
...
func (e *EnhancedStrategyEngine) maPullbackStrategy(stock *model.Stock, tradeDate time.Time) (float64, string, float64, float64, float64) {
	// 获取最近60个交易日的日线数据
...
func (e *EnhancedStrategyEngine) breakoutPullbackStrategy(stock *model.Stock, tradeDate time.Time) (float64, string, float64, float64, float64) {
	// 获取最近90个交易日的日线数据
...
func (e *EnhancedStrategyEngine) strongStockReboundStrategy(stock *model.Stock, tradeDate time.Time) (float64, string, float64, float64, float64) {
	// 获取最近60个交易日的日线数据
	startDate := tradeDate.AddDate(0, 0, -90)
	dailyData, err := e.dailyDataRepo.GetDailyData(stock.Code, startDate, tradeDate)
...
func (e *EnhancedStrategyEngine) executeMediumTermStrategy(strategy *model.Strategy, stocks []model.Stock, tradeDate time.Time) ([]model.StrategyResult, error) {
	var results []model.StrategyResult
...
func (e *EnhancedStrategyEngine) industryGrowthStrategy(stock *model.Stock, tradeDate time.Time) (float64, string, float64, float64, float64) {
	// 获取最近120个交易日的日线数据
...
func (e *EnhancedStrategyEngine) turnaroundStrategy(stock *model.Stock, tradeDate time.Time) (float64, string, float64, float64, float64) {
	// 获取最近180个交易日的日线数据
...
func (e *EnhancedStrategyEngine) dividendStrategy(stock *model.Stock, tradeDate time.Time) (float64, string, float64, float64, float64) {
	// 高股息策略需要基本面数据，当前无法实现
	// 这里提供一个基于技术面的简化版本
...
func (e *EnhancedStrategyEngine) executeLongTermStrategy(strategy *model.Strategy, stocks []model.Stock, tradeDate time.Time) ([]model.StrategyResult, error) {
...
func (e *EnhancedStrategyEngine) filterByLiquidity(stocks []model.Stock, tradeDate time.Time, minTurnover int64) []model.Stock {
...
func (e *EnhancedStrategyEngine) checkLiquidity(stockCode string, tradeDate time.Time, minTurnover int64) bool {
...
// calculateMA 计算移动平均线
func (e *EnhancedStrategyEngine) calculateMA(dailyData []model.StockDailyData, period int) float64 {
...
func (e *EnhancedStrategyEngine) calculateVolumeMA(dailyData []model.StockDailyData, period int) int64 {
	if len(dailyData) < period {
		return 0
	}
...
func (e *EnhancedStrategyEngine) analyzeVolumePattern(dailyData []model.StockDailyData) float64 {
	if len(dailyData) < 15 {
		return 0
	}

	// 检查放量上涨→缩量回踩模式
	recentData := dailyData[len(dailyData)-10:]

	// 检查前5个交易日是否放量
	volumeIncrease := false
	if len(recentData) >= 5 {
		volumeMA5 := e.calculateVolumeMA(recentData[:5], 5)
	...
	if len(recentData) >= 3 {
		recentVolume := e.calculateVolumeMA(recentData[len(recentData)-3:], 3)
		prevVolume := e.calculateVolumeMA(recentData[:3], 3)
...
func (e *EnhancedStrategyEngine) analyzeReboundVolumePattern(dailyData []model.StockDailyData) float64 {
	if len(dailyData) < 10 {
		return 0
	}
...
func (e *EnhancedStrategyEngine) analyzeCandlePattern(data model.StockDailyData) float64 {
	// 计算实体大小
...
func (e *EnhancedStrategyEngine) getClosestMA(price, ma5, ma10 float64) string {
...
func (e *EnhancedStrategyEngine) calculateBoxRange(dailyData []model.StockDailyData) (float64, float64) {
	if len(dailyData) == 0 {
		return 0, 0
	}
...
func (e *EnhancedStrategyEngine) isFirstPullback(dailyData []model.StockDailyData, maPeriod, lookback int) bool {
...
func (e *EnhancedStrategyEngine) calculateVolatility(dailyData []model.StockDailyData) float64 {
	if len(dailyData) < 2 {
		return 0
	}
